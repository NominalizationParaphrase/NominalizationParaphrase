...
dfs=pd.read_csv("FinalDatasetDatasetT5Fast.csv",index_col=0)
for index,i in enumerate(dfs['sentence']):
    ...
    if findVMpO==True and not pd.isnull(dfs.loc[index,'MVpOPassive']):
        ...
        MVpOPassivePrep=dfs.loc[index,'MVpOPassive'].split()[dfs.loc[index,'MVpOPassive'].split().index(dfs.loc[index,'pastParticipleVerb'])+1]
        findPrepWord=False
    ...
    if findVOpM==True and not pd.isnull(dfs.loc[index,'OVpMPassive']) :
        ...
        OVpMPassivePrep=dfs.loc[index,'OVpMPassive'].split()[dfs.loc[index,'OVpMPassive'].split().index(dfs.loc[index,'pastParticipleVerb'])+1]
        findPrepWord=False
...
dfs.to_excel("FinalDatasetSentenceEmbeddingsStsbRobertaLargeT5P.xlsx")